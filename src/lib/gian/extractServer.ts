// ── 서버 전용: 거래명세서/청구서 → 구조화 데이터 (Gemini 비전) ──
// /api/gian/extract 와 /api/gian/email-sync 가 공용으로 사용.

const MODELS = ["gemini-2.0-flash", "gemini-2.0-flash-001", "gemini-2.5-flash"];

const SCHEMA_PROMPT = `당신은 한국 회계 담당자입니다. 아래 거래명세서/세금계산서/청구서를 읽고 지출결의서 작성에 필요한 정보를 추출하세요.

반드시 아래 JSON 형식으로만 답하세요. 설명·코드블록 없이 JSON 객체 하나만:
{
  "vendor": "거래처(공급자) 상호명",
  "bizNo": "공급자 사업자등록번호 (예: 214-81-45210, 없으면 \\"\\")",
  "date": "거래일자/청구일자 YYYY-MM-DD (여러 날짜면 대표 발행일)",
  "items": [
...
export function normalizeStatement(raw: any) {
  ...
  return {
    ...
    items: cleanItems,
    supplyTotal, vatTotal, grandTotal,
    note: String(raw.note ?? "").trim(),
    confidence: raw.confidence ?? {},
  };
}

/** 이미지/PDF(base64) 또는 텍스트에서 명세서 구조를 추출한다. */
export async function extractStatement(input: { imageBase64?: string; mimeType?: string; text?: string }) {
  ...
  if (input.imageBase64) {
    ...
    throw new Error("이미지 또는 텍스트가 필요합니다.");
  }
  ...
  if (!response) throw new Error(lastErr.slice(0, 300) || "Gemini 호출 실패");

  const result = await response.json();
  const rparts: any[] = result.candidates?.[0]?.content?.parts ?? [];
  const outText = rparts.filter((p) => !p.thought).map((p) => p.text ?? "").join("").trim();
  if (!outText) throw new Error("인식 결과가 비어있습니다.");

  return normalizeStatement(extractJson(outText));
}
